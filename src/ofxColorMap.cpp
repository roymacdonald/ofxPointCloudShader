...
#include <numeric>
ofxColorMapGui::ofxColorMapGui(ofxGuiGroup* gui, string name, ofxColorMap* colorMap){
...
void ofxColorMapGui::setup(ofxGuiGroup* gui, string name, ofxColorMap* colorMap){
...
void ofxColorMap::setup(ofxGuiGroup* gui, string name){
    colorMapGui = make_unique<ofxColorMapGui>(gui, name, this);

}
void ofxColorMap::selectMap(size_t index){
...
void ofxColorMap::setColorMapFromData(const float data[][3], int dataLength){
    img.setFromPixels(reinterpret_cast<const float*>(&data[0][0]), dataLength, 1 ,OF_IMAGE_COLOR);
}

void ofxColorMap::addColorMap(const string& name, ofFloatPixels colorMap ){
...
vector<int> distributeBins(size_t num, size_t total, bool printVals = false){
    ...
    if(sum != total){
        ofLogError("ofxColorMap::makeColorMap") << "Sum of all values is wrong: " << sum << " should be " << total;
        return vector<int>();
    }
...
void ofxColorMap::makeColorMap(const string& name, const vector<ofFloatColor>& colors){
        
    if(colors.size() < 2){
        ofLogError("ofxColorMap::makeColorMap") << "Number of colors passed must be more than 1";
        return;
    }
    int w = 256;
